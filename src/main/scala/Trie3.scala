class Trie3[V](key: Option[Char]) {
  ...
  private var nodes = new TreeMap[Char, Trie3[V]]

  var value: Option[V] = None

  def put(k: String, v: V): Trie3[V] = {
  ...
  def nodeFor(k: String): Option[Trie3[V]] = {
    normalise(k) match {
      case Seq() => Some(this)
